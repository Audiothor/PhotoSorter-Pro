...
*   **Interface Moderne & Suivi visuel :** Construit avec `customtkinter` en mode sombre. L'application gère le redimensionnement fluide des images (y compris `.bmp`) via `Pillow`, affiche en temps réel le dossier de destination actif, permet de le **renommer à la volée**, propose des boutons d'ouverture rapide (👁️) pour afficher instantanément la source ou la destination dans votre explorateur, et offre un **guide interactif** des touches et commandes.
*   **Affichage des Fichiers Restants :** Indique sous le nombre de vidéos trouvées le nombre exact de fichiers restants hors photos et vidéos (autres types de fichiers) présents dans le dossier source.
*   **Support Étendu des Formats :** Intégration complète du format `.bmp` en plus des formats `.jpg`, `.jpeg` et `.png`, avec préservation de la qualité et gestion sécurisée des rotations de fichiers BMP.
...
| :--- | :--- | :--- |
| **Garder / Classer** | <kbd>→</kbd> ou <kbd>Entrée</kbd> | Déplace et classe la photo dans le dossier, archive l'originale. |
| **Corbeille** | <kbd>Suppr (Delete)</kbd> | Déplace la photo vers le dossier de protection `_corbeille_tri`. |
| **Passer / Suivant** | <kbd>Échap</kbd> | Ignore la photo actuelle et passe à la suivante sans rien déplacer. |
...
| :--- | :--- |
| `"garder"`, `"sauvegarder"`, `"ok"`, `"okay"`, `"oui"`, `"ouais"`, `"yes"` | Classer la photo dans le dossier en cours. |
| `"supprimer"`, `"corbeille"`, `"non"`, `"nan"`, `"no"` | Mettre à la corbeille locale de tri. |
...
C'est ici que votre nouvelle bibliothèque se construit. Le programme sépare automatiquement les photos et les vidéos dans deux sous-dossiers distincts à la racine, puis crée l'arborescence chronologique par année et par événement.
...
```
*   **Séparation automatique (`photo` / `video`) :** Trie les photos et les vidéos à la racine pour un archivage clair et professionnel.
*   **Dossier Année (`YYYY`) :** Généré automatiquement selon la date exacte du média.
...
1. **Extraction de Date (`get_safe_date`) :** Protocole à 3 niveaux.
   ...
   - *Priorité 3* : Plus petite valeur entre `os.path.getctime` (Création) et `os.path.getmtime` (Modification) pour gérer correctement les fichiers copiés sous Windows (dont la date de création est plus récente que la date de modification réelle).
