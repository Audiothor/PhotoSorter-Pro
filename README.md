...
*   **Interface Moderne & Suivi visuel :** Construit avec `customtkinter` en mode sombre. L'application gère le redimensionnement fluide des images (y compris `.bmp`) via `Pillow`, affiche en temps réel le dossier de destination actif, permet de le **renommer à la volée**, et propose un **guide interactif** des touches et commandes.
...
*   **Détection des Doublons Hybride (v1.13.29) :** Moteur d'indexation MD5 avec cache persistant (`.photosorter_index.json`) et système de détection physique en direct pour une fiabilité totale.
...
PhotoSorter Pro (v1.13.29) est développé avec une architecture claire séparant l'UI, le traitement d'image, le thread d'écoute vocale et le moteur d'indexation de doublons :
